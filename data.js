...
/* =====================================================
   CACHE LAYER — avoids re-fetching from Firestore on
   every page navigation. Data is cached in localStorage
   (shared across all pages on the same site) for a few
   minutes, then automatically refreshed.
   ===================================================== */
...
function _setCache(key, data) {
  ...
  catch (e) { /* storage full or blocked — ignore, just skip caching */ }
}

/* Call this from the admin panel after saving a product/category
   to force all customer tabs to fetch fresh data next time they load. */
function clearTajRiceCache() {
  try {
    localStorage.removeItem('taj_categories_cache');
    localStorage.removeItem('taj_products_cache');
  } catch (e) {}
}

/* ── LIVE CATEGORIES — now cached ── */
async function loadLiveCategories() {
  const cached = _getCache('taj_categories_cache');
  if (cached) { CATEGORIES = cached; window._liveCategoriesLoaded = true; return; }
  ...
  try {
    ...
    if (!snap.empty) {
      ...
      _setCache('taj_categories_cache', live);
    }
  } catch (e) {
...
/* ── LIVE PRODUCTS — shared cached loader used by index.html & products.html ── */
async function loadLiveProducts() {
  const cached = _getCache('taj_products_cache');
  if (cached) {
    window._fbAll   = cached.all;
    window._fbProds = cached.byCat;
    return { all: cached.all, byCat: cached.byCat, fromCache: true };
  }
  ...
  const existing = getApps().find(a => a.name === 'taj-products-shared');
  const app = existing || initializeApp(_FB_CONFIG_CAT, 'taj-products-shared');
  const db  = getFirestore(app);
  ...
  snap.forEach(d => {
    const raw = d.data();
    const p = { id: d.id, ...raw, price: raw.sp || raw.price || 0, section: raw.category };
    all.push(p);
  ...
  window._fbAll   = all;
  window._fbProds = byCat;
  _setCache('taj_products_cache', { all, byCat });
  return { all, byCat, fromCache: false };
}
...
function buildHeader(activePage) {
  // Ann bar close (desktop top bar)
  const annX = document.getElementById('ann-x');
  if (annX) annX.onclick = () => { const el = document.getElementById('ann'); if (el) el.remove(); };
  ...
  // ── Mobile announcement popup (replaces top bar on small screens) ──
  initMobileAnnPopup();
  ...
  if (sinput && sdrop) {
    ...
    document.addEventListener('click', e => {
      ...
      if (sw && !sw.contains(e.target) && !e.target.closest('.mobile-search-toggle')) sdrop.classList.remove('open');
    });
  }

  // ── Mobile search icon toggle ──
  const mSearchBtn = document.getElementById('mobile-search-toggle');
  const hdrSearchEl = document.querySelector('.hdr-search');
  if (mSearchBtn && hdrSearchEl) {
    mSearchBtn.onclick = () => {
      hdrSearchEl.classList.toggle('mobile-open');
      if (hdrSearchEl.classList.contains('mobile-open')) {
        const inp = document.getElementById('hdr-sinput');
        if (inp) setTimeout(() => inp.focus(), 50);
      }
    };
    document.addEventListener('click', e => {
      if (hdrSearchEl.classList.contains('mobile-open') &&
          !hdrSearchEl.contains(e.target) &&
          !e.target.closest('.mobile-search-toggle')) {
        hdrSearchEl.classList.remove('mobile-open');
      }
    });
  }
...
/* ══════════════════════════════════════
   ...
   Injects a slide-in drawer (Home / Products / About)
   once per page, wired to the hamburger button already
   present in each page's header markup.
   ══════════════════════════════════════ */
function initHamburgerMenu() {
  const btn = document.getElementById('hamburger-btn');
  if (!btn) return; // page doesn't have a hamburger button — skip

  // Inject drawer + overlay once
  if (!document.getElementById('mobile-nav-drawer')) {
    ...
    wrap.innerHTML = `
      ...
      <div class="mobile-nav-drawer" id="mobile-nav-drawer">
        <div class="mnd-brand">
          <img src="logo-intro.png" alt="Taj Rice"/>
        </div>
        <div class="mnd-item" onclick="closeHamburgerMenu();goHome()"><span class="mnd-icon">🏠</span> Home</div>
        <div class="mnd-item" onclick="closeHamburgerMenu();goProducts()"><span class="mnd-icon">📦</span> Products</div>
        <div class="mnd-item" onclick="closeHamburgerMenu();goAbout()"><span class="mnd-icon">ℹ️</span> About</div>
        <div class="mnd-item" onclick="closeHamburgerMenu();goCart()"><span class="mnd-icon">🛒</span> Cart</div>
      </div>`;
  ...
  overlay.onclick = () => window.closeHamburgerMenu();
}
...
/* ══════════════════════════════════════
   ...
   Replaces the desktop top bar with a small dismissible
   toast on mobile screens — shown once per session.
   ══════════════════════════════════════ */
function initMobileAnnPopup() {
  if (window.innerWidth > 768) return; // desktop keeps the normal top bar only
  if (sessionStorage.getItem('taj_ann_popup_seen')) return;

  if (!document.getElementById('mobile-ann-popup')) {
    const el = document.createElement('div');
    el.id = 'mobile-ann-popup';
    el.innerHTML = `
      <button class="map-close" onclick="document.getElementById('mobile-ann-popup').remove()">✕</button>
      🚚 <em>Free Delivery</em> on orders above Rs. 2,000 &nbsp;·&nbsp; 💵 Cash on Delivery`;
    document.body.appendChild(el);
    sessionStorage.setItem('taj_ann_popup_seen', '1');
